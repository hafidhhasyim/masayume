    ...
    try {
        ...
        (0, estree_walker_1.walk)(ast, {
            enter(node) {
                ...
                let developerId = `${rel}:${line}:${column}`;
                // Enhance the ID with context if we have map information
                if (mapContext) {
                    developerId += `@${mapContext.arrayName}`;
                }
                ...
                ms.appendLeft(node.name.end, ` data-developer-id="${developerId}" data-developer-name="${semanticName}"`);
                mutated = true;
            },
        });
        if (!mutated)
